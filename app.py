...
def build_cancel_done_flex_message(record, action_label):
    record_id = getvalue(record, "$id", "")
    if action_label == "処分":
        title, color, bg, status = "❌ 店舗処分で承りました", "#D32F2F", "#FFF5F5", STATUS_CANCEL_DISPOSE
...
def notify():
    ...
    if status == STATUS_ESTIMATE or "見積" in status:
        ...
        line_res = send_line_push_messages(user_id, [build_shipping_flex_message(record)])
    elif status == STATUS_DONE_STORE:
        line_res = send_line_push_messages(user_id, [build_store_pickup_flex_message(record)])
    else:
